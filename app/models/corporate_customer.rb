class CorporateCustomer < ActiveRecord::Base
  ...
  # has_many :ordinary_link_men, ->{ where('corporate_customer_link_men.corporate_customer_link_man_type_id' => 
  ...
  # has_many :manager_link_men, ->{ where('corporate_customer_link_men.corporate_customer_link_man_type_id' => 
  ...
  [:ordinary, :manager].each do |prefix|
    define_method("#{prefix}_link_men") do
      link_men.where(corporate_customer_link_man_type_id: CorporateCustomer::LinkMan::LinkType.send("#{prefix}_id"))
    end
  end
